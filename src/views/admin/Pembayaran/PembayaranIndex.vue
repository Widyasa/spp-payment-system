...
import CreatePembayaran from "@/components/Pembayaran/CreatePembayaran.vue";
import EditPembayaran from "@/components/Pembayaran/EditPembayaran.vue";
import DeletePembayaran from "@/components/Pembayaran/DeletePembayaran.vue";

const message = ref('')
const id = ref('')
const response = ref(false)
const {getAllPembayaran} = usePembayaranStore()
const {pembayaran, pembayaranList} = storeToRefs(usePembayaranStore())
const getAllModel = async ()  => {
  try {
    const response = await getAllPembayaran()
    const {data, message} = response.data
    pembayaranList.value = data.pembayarans
  } catch (e) {
...
const savePembayaranId = (pembayaranId:string) => {
  id.value = pembayaranId
}
...
<template>
  ...
  <PrimaryButton type="button" class="mt-5"  data-bs-toggle="modal" data-bs-target="#modalPembayaranCreate" title="Tambah Pembayaran" />
  ...
  <table class="table table-border">
    ...
    <tbody v-if="pembayaranList.length > 0">
    <tr v-for="(pembayaran, index) in pembayaranList" :key="pembayaran.id">
      <td>{{ index+1 }}</td>
      <td>{{ pembayaran.tahun_ajaran }}</td>
      <td>{{ pembayaran.nominal }}</td>
      <td class="d-flex gap-3">
        <PrimaryButton type="button" class="btn-warning text-white" @click="savePembayaranId(pembayaran.id)" data-bs-toggle="modal" data-bs-target="#modalPembayaranUpdate" title="Edit" />
        <PrimaryButton type="button" class="btn-danger" @click="savePembayaranId(pembayaran.id)"  data-bs-toggle="modal" data-bs-target="#modalPembayaranDelete" title="Hapus " />
      </td>
    </tr>
    </tbody>
    <tbody v-else>
  ...
  <div class="" >
    <CreatePembayaran @list-pembayaran="showForm()" @message-pembayaran="showMessage"/>
    <EditPembayaran @list-pembayaran="showForm()" @message-pembayaran="showMessage" :pembayaran-id="id"/>
    <DeletePembayaran @list-pembayaran="showForm()" @message-pembayaran="showMessage" :pembayaran-id="id"/>
  </div>
</template>
