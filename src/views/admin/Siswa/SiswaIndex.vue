...
const {getAllSiswa} = useSiswaStore()
const {getPembayaranById} = usePembayaranStore()
const {getKelasById} = useKelasStore()
const {siswa, siswaList} = storeToRefs(useSiswaStore())
...
<template>
  ...
  <RouterLink :to="{name: 'siswaCreate'}" class="mt-5">
    <primary-button title="tambah siswa" />
  </RouterLink>
  ...
  <table class="table table-border">
    ...
    <tbody v-if="siswaList.length > 0">
    <tr v-for="(siswa, index) in siswaList" :key="pembayaran.id">
      <td>{{ index+1 }}</td>
      <td>{{ siswa.nisn }}</td>
      <td>{{ siswa.nis }}</td>
      <td>{{ siswa.nama }}</td>
